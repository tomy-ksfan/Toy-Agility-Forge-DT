"""Extract ForgeNet-style transition shards from large JAX-FEM SQLite DBs.
...
def normalize_max_transitions(value: int | None) -> int | None:
    """Return ``None`` when every usable transition should be extracted.

    The command-line interface uses zero to mean no transition-count limit.
    ``None`` is the corresponding internal representation and is serialized as
    JSON ``null`` in the extraction metadata.
    """
...
def extract_db(
    ...
    max_transitions = normalize_max_transitions(max_transitions)
    ...
    for row in iter_rows(
        ...
        if (
            ...
            and stats.transitions_written >= max_transitions
        ):
            break
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract processed ForgeNet transition shards from SQLite DBs."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Maximum transitions to write per database. Use 0 for no limit.",
    )
    parser.add_argument(
        "--line-limit",
        type=int,
...
def main() -> None:
    args = parse_args()
    max_transitions = normalize_max_transitions(args.max_transitions)
    ...
    metadata = {
        ...
        "samples_per_shard": args.samples_per_shard,
        "line_limit": line_limit,
        "max_transitions_per_db": max_transitions,
        "apply_pose": not args.no_apply_pose,
        "order_by_series": args.order_by_series,
